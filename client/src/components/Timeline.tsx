    ...
    <div className="flex-1 bg-white">
      ...
      <div className="relative">
        ...
        {tasks.map((task, index) => {
          ...
          return (
            <div key={task.id} className="h-16 border-b border-wrike-border relative hover:bg-wrike-hover/30 transition-colors duration-200">
              {position >= 0 && position < numberOfDays && (
                <div 
                  className={`absolute top-4 h-8 ${barColor} rounded flex items-center px-4 text-white text-sm font-medium shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer`}
                  style={{
                    left: `${leftOffset}px`,
